...
__kernel void Update(__global float3* X, __global float3* Y, __global float* step, __global float* error, uchar border)
{
    ...
    step[0] /= pow((float)error[0], (float)0.1); 
}



__kernel void Error(__global float3* X, __global float3* Y, __global float3* F, float step, __global float* error)
{
    unsigned int ix = get_global_id(0);
    unsigned int iy = get_global_id(1);
    unsigned int iz = get_global_id(2);
    ...
    error[0] += (X[idx].x+F[idx].x-Y[idx].x)*(X[idx].x+F[idx].x-Y[idx].x) + 
                ...
                (X[idx].z+F[idx].z-Y[idx].z)*(X[idx].z+F[idx].z-Y[idx].z) /step/(float)(nx*ny*nz);
