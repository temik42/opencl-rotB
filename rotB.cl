...
__kernel void flush(__global float* X)
{    
    X[0] = 0.;    
}
...
__kernel void Step(__global float3* X, __global float3* F, __global float* step, float coeff, uchar border)
{
    unsigned int ix = get_global_id(0);
    unsigned int iy = get_global_id(1);
    unsigned int iz = get_global_id(2);
    ...
    if ((border == 0) || ((ix != 0) && (ix != (nx-1)) && (iy != 0) && (iy != (ny-1)) && (iz != 0) && (iz != (nz-1)))) {
        X[idx] += F[idx]*(float3)(step[0]*coeff);
...
__kernel void Update(__global float3* X, __global float3* Y, __global float* step, __global float* error, uchar border)
{
    unsigned int ix = get_global_id(0);
    unsigned int iy = get_global_id(1);
    unsigned int iz = get_global_id(2);
...
__kernel void AdjustStep(__global float* step, __global float* error)
{
    step[0] /= pow(error[0], 0.1f);
}


__kernel void Error(__global float3* X, __global float3* Y, __global float3* F, float scale, __global float* error)
{
    unsigned int ix = get_global_id(0);
    unsigned int iy = get_global_id(1);
    unsigned int iz = get_global_id(2);
    ...
    error[0] += ((X[idx].x+F[idx].x-Y[idx].x)*(X[idx].x+F[idx].x-Y[idx].x) + 
                (X[idx].y+F[idx].y-Y[idx].y)*(X[idx].y+F[idx].y-Y[idx].y) + 
                (X[idx].z+F[idx].z-Y[idx].z)*(X[idx].z+F[idx].z-Y[idx].z))/(scale*scale)/(float)(nx*ny*nz);
